...
import (
	douyinv2 "douyinv2/biz/handler/douyinv2"
	...
	root := r.Group("/", rootMw()...)
	{
		_douyin := root.Group("/douyin", _douyinMw()...)
		{
			_comment := _douyin.Group("/comment", _commentMw()...)
			{
				...
				_action.POST("/", append(_commentactionMw(), douyinv2.CommentAction)...)
			}
			{
				_list := _comment.Group("/list", _listMw()...)
				_list.GET("/", append(_commentlistMw(), douyinv2.CommentList)...)
			}
		}
		{
			_favorite := _douyin.Group("/favorite", _favoriteMw()...)
			{
				...
				_action0.POST("/", append(_likeactionMw(), douyinv2.LikeAction)...)
			}
			{
				_list0 := _favorite.Group("/list", _list0Mw()...)
				_list0.GET("/", append(_likevideosMw(), douyinv2.LikeVideos)...)
			}
		}
		{
			_feed := _douyin.Group("/feed", _feedMw()...)
			_feed.GET("/", append(_feed0Mw(), douyinv2.Feed)...)
		}
		{
			_publish := _douyin.Group("/publish", _publishMw()...)
			{
				_action1 := _publish.Group("/action", _action1Mw()...)
				_action1.POST("/", append(_videopublishMw(), douyinv2.VideoPublish)...)
			}
			{
				_list1 := _publish.Group("/list", _list1Mw()...)
				_list1.GET("/", append(_getvideolistMw(), douyinv2.GetVideoList)...)
			}
		}
		{
			_relation := _douyin.Group("/relation", _relationMw()...)
			{
				...
				_action2.POST("/", append(_relationactionMw(), douyinv2.RelationAction)...)
			}
			{
				_follow := _relation.Group("/follow", _followMw()...)
				{
					_list2 := _follow.Group("/list", _list2Mw()...)
					_list2.GET("/", append(_followlistMw(), douyinv2.FollowList)...)
				}
			}
			{
				_follower := _relation.Group("/follower", _followerMw()...)
				{
					...
					_list3.GET("/", append(_followerlistMw(), douyinv2.FollowerList)...)
				}
			}
			{
				_friend := _relation.Group("/friend", _friendMw()...)
				{
					...
					_list4.GET("/", append(_friendlistMw(), douyinv2.FriendList)...)
				}
			}
		}
		{
			_user := _douyin.Group("/user", _userMw()...)
			_user.GET("/", append(_userinfoMw(), douyinv2.UserInfo)...)
			{
				_login := _user.Group("/login", _loginMw()...)
				_login.POST("/", append(_loginuserMw(), douyinv2.LoginUser)...)
			}
			{
				_register := _user.Group("/register", _registerMw()...)
				_register.POST("/", append(_registeruserMw(), douyinv2.RegisterUser)...)
			}
		}
	}
}
